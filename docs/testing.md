...
| proxy_integration | 73 | Reverse proxy dispatch, route management, framework flags, API routing coverage |
...
73 tests ported from portless's test suite plus hostless-specific hardening/compatibility coverage. Organized into sections:
...
| `test_responses_websocket_mode_roundtrip_with_token` | `/v1/responses` websocket mode proxy success path in strict auth mode |
| `test_responses_websocket_mode_rejects_provider_scope` | `/v1/responses` websocket mode provider-scope rejection before upgrade |
