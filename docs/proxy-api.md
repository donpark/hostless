...
| Ollama compatibility | `/api/tags`, `/api/show`, `/api/ps` | No bridge token or admin header; request must stay on bare localhost with empty or localhost `Origin` |
...
Hostless supports a narrow subset of Ollama API endpoints for model discovery and recent-activity reporting over Hostless-managed cloud providers.

Supported Ollama endpoints:

- `GET /api/tags` lists models discovered from configured upstream providers, with `config.json` fallback/overrides, in Ollama-shaped JSON.
- `POST /api/show` returns synthetic metadata for a discovered model.
- `GET /api/ps` returns models with recent successful proxy activity, not true local in-memory residency.

This is intentionally not a claim of full Ollama API compatibility. Hostless does not currently implement Ollama generation endpoints such as `POST /api/chat` or `POST /api/generate`.

Access rules:

- no bridge token required
- no `x-hostless-admin` header required
- request `Host` must stay on bare localhost when present
- request `Origin` must be empty or bare localhost
...
| `GET` | `/api/tags` | Ollama-compatible model discovery for configured providers |
| `POST` | `/api/show` | Ollama-compatible synthetic metadata lookup for one model |
| `GET` | `/api/ps` | Ollama-compatible recent-activity view of models used through Hostless |
...
Returns a Hostless-owned response for one of the supported Ollama API endpoints: `GET /api/tags`.
...
- model names are provider-qualified
- list output is filtered to providers with configured credentials
- Hostless prefers live provider discovery when the upstream supports it
- `~/.hostless/config.json` can supply fallback models or override synthetic metadata under `ollama_api_models`
- the external catalog uses `gemini/...` for Google Gemini models
- metadata is synthetic and Hostless-owned rather than pulled from local Ollama artifacts

### `POST /api/show`

Returns a Hostless-owned response for one of the supported Ollama API endpoints: `POST /api/show`.

Example request:

```bash
curl -X POST "http://localhost:${PORT}/api/show" \
...
```json
{
  "license": "Hosted by Hostless",
  "modelfile": "# Synthetic Modelfile generated by Hostless\nFROM openai/gpt-4o\nPARAMETER provider openai\nPARAMETER context_window 128000",
  ...
  "model_info": {
    "hostless.provider": "openai",
    "hostless.provider_key": "openai",
    "hostless.model_id": "gpt-4o",
    "hostless.context_window": 128000,
    "hostless.synthetic": true
  },
...
Returns a Hostless-owned response for one of the supported Ollama API endpoints: `GET /api/ps`.

Important semantic difference from Ollama:

- this is **not** a true list of models loaded into local memory
- it is a short-lived recent-activity window over models used through Hostless

Example request:

```bash
curl "http://localhost:${PORT}/api/ps"
```

Example response after recent use:
  ...
  "models": [
    {
      ...
      "modified_at": "2024-07-18T00:00:00Z",
      "size": 128000,
      "digest": "hostless:openai/gpt-4o-mini",
      "details": {
        "parent_model": "",
        "format": "api",
        "family": "gpt",
      ...
      "expires_at": "2026-03-10T12:34:56Z",
      "size_vram": 0
    }
  ]
}
