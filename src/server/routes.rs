...
use super::ollama::OllamaModelSpec;
use super::streaming;
use super::AppState;
...
fn ensure_local_ollama_access(headers: &HeaderMap) -> Result<(), Response> {
    let host = headers
        .get(header::HOST)
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    if !host.is_empty() && !is_bare_localhost_host(host) {
        return Err((
            StatusCode::FORBIDDEN,
            Json(json!({
                "error": "Ollama compatibility endpoints are only available on bare localhost"
            })),
        )
            .into_response());
    }
    ...
    Err((
        StatusCode::FORBIDDEN,
        Json(json!({
            "error": "Ollama compatibility endpoints are only available from localhost"
        })),
    )
        .into_response())
}

async fn configured_provider_names(state: &AppState) -> Result<Vec<String>, Response> {
    match state.vault.list_providers().await {
        Ok(providers) => Ok(providers.into_iter().map(|provider| provider.name).collect()),
        Err(error) => {
            error!(error = %error, "Failed to inspect configured providers for Ollama compatibility layer");
            Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({
                    "error": "Failed to inspect configured providers"
                })),
            )
                .into_response())
        }
...
fn serialize_ollama_tag(spec: &OllamaModelSpec) -> Value {
    json!({
        ...
        "digest": spec.digest,
        "details": {
            "parent_model": spec.details.parent_model.clone(),
            "format": spec.details.format.clone(),
            "family": spec.details.family.clone(),
            "families": spec.details.families.clone(),
            "parameter_size": spec.details.parameter_size.clone(),
            "quantization_level": spec.details.quantization_level.clone(),
        }
    })
}
...
pub async fn ollama_tags(
    ...
    if let Err(resp) = ensure_local_ollama_access(&headers) {
        return resp;
    }

    let configured = match configured_provider_names(&state).await {
    ...
    let models = state
        .ollama_registry
        .list_for_providers(&state, &configured)
        .await
        .into_iter()
        .map(|spec| serialize_ollama_tag(&spec))
        .collect::<Vec<_>>();
...
pub async fn ollama_show(
    ...
    let Some(spec) = state
        .ollama_registry
        .find_for_providers(&state, &configured, &req.model)
        .await
    else {
        return (
    ...
    Json(json!({
        ...
        "modelfile": format!(
            "# Synthetic Modelfile generated by Hostless\nFROM {}\nPARAMETER provider {}\nPARAMETER context_window {}",
            spec.qualified_name(),
            spec.provider_name,
            spec.context_window,
        ),
        "parameters": format!("provider {}\ncontext_window {}", spec.provider_name, spec.context_window),
        "template": "",
        "system": "",
        "details": {
            "parent_model": spec.details.parent_model.clone(),
        ...
        "model_info": {
            "hostless.provider": spec.provider_name,
            "hostless.provider_key": spec.provider_key,
            "hostless.model_id": spec.id,
            "hostless.context_window": spec.context_window,
...
pub async fn ollama_ps(
    ...
    for active in state.ollama_activity.active_models().await {
        if let Some(spec) = state
            .ollama_registry
            .find_for_providers(&state, &configured, &active.model_name)
            .await
        {
            let mut model = serialize_ollama_tag(&spec);
            model["expires_at"] = json!(active.expires_at.to_rfc3339());
            model["size_vram"] = json!(0);
            models.push(model);
        }
    }
...
async fn proxy_openai_media(
    ...
    if upstream_response.status().is_success() {
        if let Some(model) = scoped_model.as_deref() {
            state
                .ollama_activity
                .mark_active(super::ollama::canonical_model_name(model))
                .await;
        }
    }
